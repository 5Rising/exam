...
public class ExamManageController {
    ...
    public ApiResult findAlls(@PathVariable("page") Integer page, @PathVariable("size") Integer size){
        ApiResult apiResult;
        Page<ExamManage> examManage = new Page<>(page,size);
        IPage<ExamManage> all = examManageService.findAlls(examManage);
        apiResult = ApiResultHandler.buildApiResult(200, "请求成功！", all);
        return apiResult;
    }
...
//        }
    ...
    @GetMapping("/exams/{institute}/{page}/{size}")
    public ApiResult findAllByInstitute(@PathVariable("institute") String institute, @PathVariable("page") Integer page, @PathVariable("size") Integer size){
        System.out.println("分页查询所有试卷,根据学院");
        ApiResult apiResult;
        Page<ExamManage> examManage = new Page<>(page,size);
        IPage<ExamManage> all = examManageService.findAllByInstitute(examManage,institute);
        apiResult = ApiResultHandler.buildApiResult(200, "请求成功！", all);
        return apiResult;
    }

    @GetMapping("/exams/{institute}")
    public ApiResult findAllByInstitute(@PathVariable("institute") String institute){
        System.out.println("不分页查询所有试卷,根据学院");
        ApiResult apiResult;
        List<ExamManage> all = examManageService.findAllByInstitute(institute);
        apiResult = ApiResultHandler.buildApiResult(200, "请求成功！", all);
        return apiResult;
    }
}
