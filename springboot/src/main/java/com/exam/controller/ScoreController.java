...
//    不分页
    ...
    @GetMapping("/scores/{examCode}/{studentId}")
    public ApiResult findByExamCodeAndStudentId(@PathVariable("examCode") Integer examCode,@PathVariable("studentId") Integer studenId){
        List<Score> byExamCodeAndStudentId = scoreService.findByExamCodeAndStudentId(examCode, studenId);
        return ApiResultHandler.buildApiResult(200,"查询成功",byExamCodeAndStudentId);
    }
}
